...
export function convertGooglePersonToContact(
  connection: People.Schema$Person
): Contact | null {
  ...
  const contactUrl = contactId
    ? `https://contacts.google.com/contact/${contactId}`
    : null;
...
export function convertContactToGooglePerson(
  ...
  person.phoneNumbers = contact.phoneNumbers
    ...
    .map((entry: any): People.Schema$PhoneNumber => {
...
function getGooglePersonResourceId(
  connection: People.Schema$Person
): string | null {
...
function getGoogleContactName(
  connection: People.Schema$Person
): ContactName | null {
  if (!connection.names) {
    return null;
  }
  const name = connection.names.find((entry) =>
    isGoogleContactField(entry.metadata)
  );
  if (!name) {
    return null;
  }
...
function getGoogleContactOrganization(
  connection: People.Schema$Person
): string | null {
  if (!connection.organizations) {
    return null;
  }
  const organization = connection.organizations.find((entry) =>
    isGoogleContactField(entry.metadata)
  );
  if (!organization) {
    return null;
...
function getGoogleContactPhoneNumbers(
  connection: People.Schema$Person
): PhoneNumber[] {
  ...
  for (const phoneNumber of connection.phoneNumbers) {
    ...
    const phoneNumberLabel = getPhoneNumberLabel(
      phoneNumber.type
    ) as unknown as PhoneNumberLabel;
...
function getGoogleContactEmailAddress(
  connection: People.Schema$Person
): string | null {
  ...
  const contactEmailAddress = emailAddresses.find((entry) =>
    isGoogleContactField(entry.metadata)
  );
...
function getGoogleContactPhoto(
  connection: People.Schema$Person
): string | null {
